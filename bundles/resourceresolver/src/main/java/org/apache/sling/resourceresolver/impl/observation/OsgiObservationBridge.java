...
public class OsgiObservationBridge implements ResourceChangeListener, ExternalResourceListener {
    ...
    private void sendOsgiEvent(ResourceChange change) {
        ...
        if (!ArrayUtils.isEmpty(change.getAddedAttributeNames())) {
            props.put(SlingConstants.PROPERTY_ADDED_ATTRIBUTES, change.getAddedAttributeNames());
        }
        if (!ArrayUtils.isEmpty(change.getChangedAttributeNames())) {
            props.put(SlingConstants.PROPERTY_CHANGED_ATTRIBUTES, change.getChangedAttributeNames());
        }
        if (!ArrayUtils.isEmpty(change.getRemovedAttributeNames())) {
            props.put(SlingConstants.PROPERTY_REMOVED_ATTRIBUTES, change.getRemovedAttributeNames());
        }
